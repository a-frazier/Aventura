  ...
  <div class="space-y-1.5" transition:slide={{ duration: 150 }}>
    {#each issues as issue, index (issue.start + '-' + issue.end)}
      ...
      >
        <!-- Issue header -->
        <div
          class="w-full flex items-start gap-2 p-2 text-left hover:bg-yellow-900/10 transition-colors cursor-pointer"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="text-xs font-medium text-yellow-400 bg-yellow-900/40 px-1.5 py-0.5 rounded">
                {issue.kind}
              </span>
              <span class="text-xs text-yellow-300/80 truncate">
                "{issue.problemText}"
              </span>
            </div>
            <p class="text-xs text-yellow-200/70 mt-0.5 line-clamp-1">
              {issue.message}
            </p>
          </div>
          <button
            class="p-1 rounded hover:bg-yellow-800/30 text-yellow-500/60 hover:text-yellow-400 shrink-0"
        ...
        {#if expandedIssue === index}
          <div class="px-2 pb-2 pt-1 border-t border-yellow-700/30" transition:slide={{ duration: 150 }}>
            {#if issue.suggestions.length > 0}
              <div class="flex flex-wrap gap-1.5">
                {#each issue.suggestions.slice(0, 5) as suggestion, suggestionIndex}
                  <button
                    class="flex items-center gap-1 px-2 py-1 text-xs rounded bg-green-900/30 border border-green-700/40 text-green-300 hover:bg-green-900/50 transition-colors"
                ...
                {#if issue.kind.toLowerCase().includes('spell')}
                  <button
                    class="flex items-center gap-1 px-2 py-1 text-xs rounded bg-surface-700 border border-surface-600 text-surface-300 hover:bg-surface-600 transition-colors"
