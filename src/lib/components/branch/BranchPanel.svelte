...
<div class="space-y-3">
  ...
  {#snippet branchItem(branch: Branch)}
    ...
    <div class="ml-4">
      <div
        class="flex items-center gap-2 rounded-lg p-2 cursor-pointer transition-colors group {isCurrent(branch.id) ? 'bg-accent-500/20 border-l-2 border-accent-500' : 'hover:bg-surface-700/50'}"
      ...
      >
        ...
        {#if renamingBranchId === branch.id}
          <input
            type="text"
            class="input flex-1 text-sm py-0.5 px-1"
        ...
        {:else}
          ...
          <span class="text-xs text-surface-500">{getBranchEntryCount(branch.id)}</span>
  ...
  <div class="space-y-1">
    <!-- Main Branch -->
    <div
      class="flex items-center gap-2 rounded-lg p-2 cursor-pointer transition-colors {isCurrent(null) ? 'bg-accent-500/20 border-l-2 border-accent-500' : 'hover:bg-surface-700/50'}"
    ...
    >
      ...
      <GitBranch class="h-4 w-4 text-surface-400" />
      <span class="flex-1 text-sm text-surface-200">Main</span>
      <span class="text-xs text-surface-500">{getBranchEntryCount(null)}</span>
      {#if isCurrent(null)}
        <span class="h-2 w-2 rounded-full bg-accent-500" title="Current branch"></span>
      {/if}
    </div>
  ...
  {#if story.branches.length === 0}
    <p class="text-center text-sm text-surface-400 py-4">
      {#if canCreateBranch}
